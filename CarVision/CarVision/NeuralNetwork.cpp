...
void NeuralNetwork::Train(const std::vector<std::string>& imagePaths, const std::vector<int>& labels)
{
    ...
    for (int epoch = 0; epoch < numEpochs; ++epoch)
    {
        ...
        {
            auto input = LoadImage(imagePaths[i]);
            ...
            std::cout << "Processed image " << (i + 1) << "/" << imagePaths.size() << ", Loss: " << loss << std::endl;
        }
    }
}
